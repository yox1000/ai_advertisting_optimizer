...
The Test preset dropdown can fill the prompt rows, provider settings, run settings, market default, default context, protected-fact placeholders, and blocked-claim defaults for the staged tests: single-prompt debug, five-prompt set, FiDi Mezzanine multi-iteration, cross-model baseline, and cross-model edit. Presets do not replace discovered options; those remain editable before saving.
